  ...
  const uid = () => Date.now() + Math.floor(Math.random() * 1000);
    ...
    function render() {
        ...
        for (const tx of [...transactions].reverse()) {
            ...
            d.className = 'tx-desc'; d.textContent = tx.desc;
            const t = document.createElement('div');
            t.className = 'tx-time';
            t.textContent = new Date(tx.time).toLocaleString();
    ...
    function addTransaction(desc, amount, type) {
        ...
        if (type === 'expense' && tx.amount > balance) {
            showModal();
            return false;
        }
    ...
    function loadTheme() {
        ...
        const t = localStorage.getItem(THEME_KEY);
            if (t === 'dark') {
                ...
                body.classList.remove('dark-mode'); themeIcon.className = 'fa-solid fa-moon';
            }
            else {
    ...
    transactionForm.addEventListener('submit', e => {
        e.preventDefault();
        const desc = descInput.value, rawAmt = amountInput.value;
        if (!desc || rawAmt === '') {
        ...
        const ok = addTransaction(desc, Math.abs(amt), type);
        if (ok) {
